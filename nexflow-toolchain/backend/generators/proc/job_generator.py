...
class JobGeneratorMixin(
    JobImportsMixin, JobOperatorsMixin, JobCorrelationMixin, JobSinksMixin
):
    """
 ...
 */"""
    ...
    def _generate_main_method(
        self, process: ast.ProcessDefinition, class_name: str
    ) -> str:
    ...
    def _generate_build_pipeline_method(self, process: ast.ProcessDefinition) -> str:
        ...
        # Generate source connections (process.receives is direct list)
        if process.receives:
        ...
        # Generate sinks (process.emits is direct list)
        if process.emits:
            lines.append("")
            lines.append("        // Sinks")
    ...
    def _generate_source_with_json(
        ...
        if (
            ...
            and process.execution.time.watermark
        ):
            watermark_delay = str(
                duration_to_ms(process.execution.time.watermark.delay)
            )
