...
class StreamingVisitorMixin:
    ...
    def visitStreamingBlock(
        self, ctx: SchemaDSLParser.StreamingBlockContext
    ) -> ast.StreamingBlock:
    ...
    def _process_streaming_decl(
        self, ctx: SchemaDSLParser.StreamingDeclContext, streaming: ast.StreamingBlock
    ):
        if ctx.keyFieldsDecl():
            ...
            streaming.time_semantics = self.visitTimeSemanticsDecl(
                ctx.timeSemanticsDecl()
            )
            ...
            streaming.allowed_lateness = self.visitAllowedLatenessDecl(
                ctx.allowedLatenessDecl()
            )
        elif ctx.idleDecl():
    ...
    def visitKeyFieldsDecl(
        self, ctx: SchemaDSLParser.KeyFieldsDeclContext
    ) -> List[str]:
        return self._get_field_list(ctx.fieldArray())

    def visitTimeFieldDecl(
        self, ctx: SchemaDSLParser.TimeFieldDeclContext
    ) -> ast.FieldPath:
        # time_field can be fieldPath OR timeSemanticsType (like event_time)
        if ctx.fieldPath():
    ...
    def visitTimeSemanticsDecl(
        self, ctx: SchemaDSLParser.TimeSemanticsDeclContext
    ) -> ast.TimeSemantics:
    ...
    def _process_watermark_decl(
        self, ctx: SchemaDSLParser.WatermarkDeclContext, streaming: ast.StreamingBlock
    ):
        if ctx.duration():
            streaming.watermark_delay = self.visitDuration(ctx.duration())
        if ctx.watermarkStrategy():
            streaming.watermark_strategy = self.visitWatermarkStrategy(
                ctx.watermarkStrategy()
            )

    def visitWatermarkStrategy(
        self, ctx: SchemaDSLParser.WatermarkStrategyContext
    ) -> ast.WatermarkStrategy:
        ...
        return strategy_map.get(
            strategy_text, ast.WatermarkStrategy.BOUNDED_OUT_OF_ORDERNESS
        )

    def _process_late_data_decl(
        self, ctx: SchemaDSLParser.LateDataDeclContext, streaming: ast.StreamingBlock
    ):
        if ctx.lateDataStrategy():
            streaming.late_data_handling = self.visitLateDataStrategy(
                ctx.lateDataStrategy()
            )
        if ctx.IDENTIFIER():
            streaming.late_data_stream = ctx.IDENTIFIER().getText()

    def visitLateDataStrategy(
        self, ctx: SchemaDSLParser.LateDataStrategyContext
    ) -> ast.LateDataStrategy:
        strategy_text = self._get_text(ctx).lower()
        strategy_map = {
    ...
    def visitAllowedLatenessDecl(
        self, ctx: SchemaDSLParser.AllowedLatenessDeclContext
    ) -> ast.Duration:
        return self.visitDuration(ctx.duration())

    def _process_idle_decl(
        self, ctx: SchemaDSLParser.IdleDeclContext, streaming: ast.StreamingBlock
    ):
        if ctx.duration():
    ...
    def visitIdleBehavior(
        self, ctx: SchemaDSLParser.IdleBehaviorContext
    ) -> ast.IdleBehavior:
    ...
    def visitSparsityDecl(
        self, ctx: SchemaDSLParser.SparsityDeclContext
    ) -> ast.SparsityBlock:
    ...
    def visitRetentionBlockDecl(
        self, ctx: SchemaDSLParser.RetentionBlockDeclContext
    ) -> ast.RetentionOptions:
    ...
    def visitRetentionPolicy(
        self, ctx: SchemaDSLParser.RetentionPolicyContext
    ) -> ast.RetentionPolicy:
