...
Updated for grammar which uses emitDecl directly in bodyContent
...
class ProcOutputVisitorMixin:
    ...
    # =========================================================================
    # Emit Declaration (direct in bodyContent)
    # =========================================================================
    ...
    def visitPersistClause(
        self, ctx: ProcDSLParser.PersistClauseContext
    ) -> ast.PersistDecl:
        """
        ...
        for opt_ctx in ctx.persistOption():
            ...
            if opt_text == "async":
                ...
                error_handler = self.visitPersistErrorAction(
                    opt_ctx.persistErrorAction()
                )
    ...
    def visitPersistErrorAction(
        self, ctx: ProcDSLParser.PersistErrorActionContext
    ) -> ast.PersistErrorHandler:
        """
        ...
        if "emit" in action_text:
            ...
            return ast.PersistErrorHandler(
                action=ast.PersistErrorAction.FAIL, location=self._get_location(ctx)
            )
        else:
            # Default: continue
            return ast.PersistErrorHandler(
                action=ast.PersistErrorAction.CONTINUE, location=self._get_location(ctx)
            )
    ...
    def visitFanoutDecl(self, ctx: ProcDSLParser.FanoutDeclContext) -> ast.FanoutDecl:
        ...
        return ast.FanoutDecl(strategy=strategy, location=self._get_location(ctx))

    # =========================================================================
    # Completion Block
    # =========================================================================

    def visitCompletionBlock(
        self, ctx: ProcDSLParser.CompletionBlockContext
    ) -> ast.CompletionBlock:
        on_commit = None
        ...
        for decl_ctx in ctx.completionDecl():
            if decl_ctx.onCommitDecl():
                ...
                on_commit_failure = self.visitOnCommitFailureDecl(
                    decl_ctx.onCommitFailureDecl()
                )
    ...
    def visitOnCommitDecl(
        self, ctx: ProcDSLParser.OnCommitDeclContext
    ) -> ast.OnCommitDecl:
    ...
    def visitOnCommitFailureDecl(
        self, ctx: ProcDSLParser.OnCommitFailureDeclContext
    ) -> ast.OnCommitFailureDecl:
        target = ctx.IDENTIFIER().getText()
        correlation = self.visitCorrelationDecl(ctx.correlationDecl())
    ...
    def visitCorrelationDecl(
        self, ctx: ProcDSLParser.CorrelationDeclContext
    ) -> ast.CorrelationDecl:
    ...
    def visitIncludeDecl(
        self, ctx: ProcDSLParser.IncludeDeclContext
    ) -> ast.IncludeDecl:
        fields = self._get_field_list(ctx.fieldList())
        return ast.IncludeDecl(fields=fields, location=self._get_location(ctx))
