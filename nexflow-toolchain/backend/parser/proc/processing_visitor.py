...
class ProcProcessingVisitorMixin:
    ...
    def visitProcessingBlock(
        self, ctx: ProcDSLParser.ProcessingBlockContext
    ) -> Union[
        ast.EnrichDecl,
        ast.TransformDecl,
        ast.RouteDecl,
    ...
    def visitTransformDecl(
        self, ctx: ProcDSLParser.TransformDeclContext
    ) -> ast.TransformDecl:
        ...
        if hasattr(ctx, "onSuccessBlock") and ctx.onSuccessBlock():
            ...
            if (
                hasattr(success_block, "actionContent")
                and success_block.actionContent()
            ):
        ...
        if hasattr(ctx, "onFailureBlock") and ctx.onFailureBlock():
            ...
            if (
                hasattr(failure_block, "actionContent")
                and failure_block.actionContent()
            ):
    ...
    def _parse_action_content(self, action_content) -> list:
        ...
        if hasattr(action_content, "emitDecl"):
            ...
            if emits:
                ...
                for emit in emits:
                    parsed = (
                        self.visitEmitDecl(emit)
                        if hasattr(self, "visitEmitDecl")
                        else None
                    )
                    if parsed:
                        result.append(parsed)
    ...
    def visitRouteDecl(self, ctx: ProcDSLParser.RouteDeclContext) -> ast.RouteDecl:
        ...
        return ast.RouteDecl(
            rule_name=rule_name, condition=condition, location=self._get_location(ctx)
        )

    def visitAggregateDecl(
        self, ctx: ProcDSLParser.AggregateDeclContext
    ) -> ast.AggregateDecl:
        transform_name = self._get_identifier_text(ctx.IDENTIFIER())
        return ast.AggregateDecl(
            transform_name=transform_name, location=self._get_location(ctx)
        )
    ...
    def visitMergeDecl(self, ctx: ProcDSLParser.MergeDeclContext) -> ast.MergeDecl:
        ...
        output_alias = None
        has_into = any(self._get_text(child) == "into" for child in ctx.getChildren())
        if has_into and len(streams) > 2:
            output_alias = streams[-1]
            streams = streams[:-1]

        return ast.MergeDecl(
            streams=streams, output_alias=output_alias, location=self._get_location(ctx)
        )
    ...
    def visitWindowDecl(self, ctx: ProcDSLParser.WindowDeclContext) -> ast.WindowDecl:
        ...
        Grammar WINDOW windowType duration windowBody?
        ...
        # windowOptions is now inside windowBody
        options = None
        key_by = None
        if ctx.windowBody():
    ...
    def visitWindowOptions(
        self, ctx: ProcDSLParser.WindowOptionsContext
    ) -> ast.WindowOptions:
        lateness = None
        late_data = None
        ...
        return ast.WindowOptions(lateness=lateness, late_data=late_data)
    ...
    def visitEvaluateStatement(self, ctx) -> ast.EvaluateDecl:
        ...
        expression = (
            self._get_text(ctx.expression())
            if hasattr(ctx, "expression") and ctx.expression()
            else ""
        )
        return ast.EvaluateDecl(expression=expression, location=self._get_location(ctx))
    ...
    def visitTransitionStatement(self, ctx) -> ast.TransitionDecl:
        ...
        return ast.TransitionDecl(
            target_state=target_state, location=self._get_location(ctx)
        )
    ...
    def visitEmitAuditStatement(self, ctx) -> ast.EmitAuditDecl:
        ...
        return ast.EmitAuditDecl(
            event_name=event_name, location=self._get_location(ctx)
        )

    def visitDeduplicateStatement(self, ctx) -> ast.DeduplicateDecl:
        """Visit deduplicate statement."""
        key_field = (
            self._get_text(ctx.fieldPath())
            if hasattr(ctx, "fieldPath") and ctx.fieldPath()
            else ""
        )
        return ast.DeduplicateDecl(
            key_field=key_field, location=self._get_location(ctx)
        )

    def visitLookupStatement(self, ctx) -> ast.LookupDecl:
        """Visit lookup statement."""
        source_name = (
            self._get_identifier_text(ctx.IDENTIFIER()) if ctx.IDENTIFIER() else ""
        )
        return ast.LookupDecl(source_name=source_name, location=self._get_location(ctx))
    ...
    def visitBranchStatement(self, ctx) -> ast.BranchDecl:
        ...
        return ast.BranchDecl(
            branch_name=branch_name, body=body, location=self._get_location(ctx)
        )
    ...
    def visitParallelStatement(self, ctx) -> ast.ParallelDecl:
        ...
        if hasattr(ctx, "parallelBranch"):
            ...
            if branch_list:
                branch_list = (
                    branch_list if isinstance(branch_list, list) else [branch_list]
                )
        ...
        return ast.ParallelDecl(
            name=name, branches=branches, location=self._get_location(ctx)
        )
    ...
    def visitValidateInputStatement(self, ctx) -> ast.ValidateInputDecl:
        ...
        return ast.ValidateInputDecl(
            expression=expression, location=self._get_location(ctx)
        )

    def visitCallStatement(self, ctx) -> ast.CallDecl:
        """Visit call statement."""
        target = self._get_identifier_text(ctx.IDENTIFIER())
        return ast.CallDecl(target=target, location=self._get_location(ctx))
    ...
    def visitScheduleStatement(self, ctx) -> ast.ScheduleDecl:
        ...
        return ast.ScheduleDecl(
            delay=delay, target=target, location=self._get_location(ctx)
        )
    ...
    def visitSetStatement(self, ctx) -> ast.SetDecl:
        ...
        return ast.SetDecl(
            variable=variable, value=value, location=self._get_location(ctx)
        )
