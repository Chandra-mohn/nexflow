...
Extended for business date, markers, and phases support.
...
class ProcCoreVisitorMixin:
    ...
    def visitProgram(self, ctx: ProcDSLParser.ProgramContext) -> ast.Program:
        ...
        return ast.Program(
            processes=processes, imports=imports, location=self._get_location(ctx)
        )

    def visitImportStatement(
        self, ctx: ProcDSLParser.ImportStatementContext
    ) -> ImportStatement:
    ...
    def visitProcessDefinition(
        self, ctx: ProcDSLParser.ProcessDefinitionContext
    ) -> ast.ProcessDefinition:
            ...
            # Or traditional body content
                ...
                for body_ctx in body_or_phases.bodyContent():
                    if body_ctx.receiveDecl():
                        ...
                        processing.append(
                            self.visitProcessingBlock(body_ctx.processingBlock())
                        )
                        ...
                        completions.append(
                            self.visitCompletionBlock(body_ctx.completionBlock())
                        )
        ...
        if ctx.processTailBlocks() and ctx.processTailBlocks().resilienceBlock():
            resilience = self.visitResilienceBlock(
                ctx.processTailBlocks().resilienceBlock()
            )
