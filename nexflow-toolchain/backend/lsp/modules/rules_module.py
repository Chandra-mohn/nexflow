...
class RulesModule(LanguageModule):
    ...
    KEYWORDS = {
        ...
        # EOD Marker Conditions
        "marker_conditions": ["marker", "fired", "pending", "between"],
        # Actions
    ...
    def _get_contextual_keywords(
        ...
        if current_line.startswith("rule"):
            return ["if", "then", "elseif", "else", "endif", "end"] + list(
                self.KEYWORDS["logic"]
            )
        ...
        try:
            ...
            return types.DocumentSymbol(
                ...
                selection_range=self.create_range(
                    start_line, 0, start_line, len(name) + 20
                ),
    ...
    def _create_rule_symbol(self, rule, content: str) -> Optional[types.DocumentSymbol]:
        ...
        try:
            ...
            return types.DocumentSymbol(
                ...
                range=rule_range,
                selection_range=self.create_range(
                    start_line, 0, start_line, len(name) + 10
                ),
                detail="rule",
            )
        except Exception:
